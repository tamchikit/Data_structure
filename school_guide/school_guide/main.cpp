...
//  school_guide
//
//  Created by tamchikit on 2018/11/13.
...
void CostleastPath_DIJ(MGraph G, int v0, PathMatrix &P, ShortPathTable &D){
    int v;
    Status final[MAX_VERTEX_NUM];
    for (v=0; v<G.vexnum; v++) {
        final[v]=FALSE;
    ...
    while (key!=-1) {
        ...
        if (key==1) {
            while (choice1!=-1) {
                cout<<"*****请输入想查询的城市（-1则结束）：\n";
                cin>>choice1;
                cout<<"*****城市名称："<<G.vexs[choice1-1].name<<"\n*****城市介绍："<<G.vexs[choice1-1].information<<endl;
            }
        }
        if (key==2) {
            while (choice2!=-1) {
        ...
        if (key==3) {
            while (choice2!=-1) {
                ...
                cin>>choice3;
                if (choice3!=choice2&&choice2<G.vexnum&&choice3<G.vexnum&&choice2>=0&&choice3>=0) {
                    cout<<"******"<<G.vexs[choice2].name<<"到"<<G.vexs[choice3].name<<"的最少花费路线为：";
                    ShortestPath_DIJ(G, choice2, P, D);
                    for (int i=0; i<G.vexnum; i++) {
                        if (P[choice3][i]) {
                            cout<<G.vexs[i].name;
